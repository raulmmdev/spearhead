...
class Product implements Rule
{
    ...
    {
        $validStatuses = [SiteProduct::STATUS_ENABLED, SiteProduct::STATUS_DISABLED];

        $this->validator = Validator::make($value, [
            'product_id' => ['required', 'integer', ExtendedRule::unique('site_product', 'source_id')->where(function($query) {

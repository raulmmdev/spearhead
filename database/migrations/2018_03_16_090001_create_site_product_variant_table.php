...
class CreateSiteProductVariantTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('site_product_variant', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('site_id')->nullable(false)->unsigned()->comment('Site id');
            $table->integer('site_product_id')->nullable(false)->unsigned()->comment('Site product id');
            ...
            $table->foreign('site_product_id')->references('id')->on('site_product')->onDelete('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('site_product_variant');
    }
}
